...
public class ScEmployeeDao implements IScEmployee
{
    ...
    {
        List<ScTurn> result = null;
        try
        {
            Query query = entityManager.createNamedQuery("ScTurn.findAll");
            result = (List<ScTurn>) query.getResultList();
        }
        catch (Exception e)
        {
            log.error("Error al intentar consultar los turnos de los empleados", e);
        }
        return result;
    }
